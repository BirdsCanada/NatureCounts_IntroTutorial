...
```{r meta, eval = FALSE}
iba<-meta_iba_codes() # retrieve the IBA metadata 
View(iba) # explore the dataframe 
```
...
ontario_birds <- nc_count(region=list(statprov="ON")) # filter nc_count by region (statprov)
View(ontario_birds) # explore the dataframe
```
...
chickadee_IDs <- search_species("chickadee") # retrieve all chickadee species_id
View(chickadee_IDs) # explore the dataframe 
```
...
```{r}
species_codes <- search_species_code() # retrieve all species codes
View(species_codes) # explore the dataframe
```

```{r species code search example, eval=FALSE}
meta_bcch <- search_species_code("BCCH") # Filter by species code
View(meta_bcch) # View the metadata for the Black-capped Chickadee (BCCH)
```
