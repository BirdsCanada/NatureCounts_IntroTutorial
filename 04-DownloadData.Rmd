...
WPWI1<-nc_data_dl(collections="WPWI", username = "sample", info="tutorial example")
```
...
WPWI_filter<-nc_data_dl(collections="WPWI", years=c(2010, 2012), region=list(bcr="13"), username = "sample", info="tutorial example")
```
...
WPWI_bb<-nc_data_dl(collections="WPWI", region = list(bbox = c(left = -81.7, bottom = 44.5, right = -80.9, top = 45.3)), username = "sample", info="tutorial example")
```
