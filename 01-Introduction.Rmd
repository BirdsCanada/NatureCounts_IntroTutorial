...
## 1.1 NatureCounts and R {#introR}

The natuecounts R package was create to allow users to directly access and download data from NatureCounts into R. The goal of this online book is to show NatureCounts users how to use the R statistical programming language (https://www.r-project.org/) to access, filter, summarize, and visualize naturecounts data. Moving forward, we intend to create more advanced turtoials which explore analytical procedures suitable for various datasets. If you have suggestions for additional content, please let us know by emailing dethier@birdscanada.org

Throughout this book we use fictious and subsets real datasets to illustrate how to access, manage, explore and visualize NaturecCounts data in R. We recommend that you run through the sample code in each chapter with the sample dataset before doing the exercises or rusing your own data. 

## 1.2 Prerequisites

This book assumes that you have a basic understanding of R. We recommend that you become familiar with ‘R for Data Science’ by Garrett Grolemund and Hadley Wickham (http://r4ds.had.co.nz/), which covers how to import, visualize, and summarize data in R using the tidyverse collection of R packages (https://www.tidyverse.org/). 

## 1.3 Acknowledgements

Some of the text included in this book was adapted from the NatureCounts GitHub repository developed by Steffi LaZerte: https://github.com/BirdStudiesCanada/naturecounts. Many people have worked together to make the NatureCounts web interface and the R-package, including Denis Lepage, Steffi LaZerte, Catherine Jardine.

The development of the NatureCounts web interface, R package, and accompanying online book were made possible through the generous contributions by Environment and Climate Change Canada.
