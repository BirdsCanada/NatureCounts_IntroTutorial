...
# to retrieve the metadata of a specific set of collections based on its project_id, you can index the 'collections' dataframe
...
When using the naturecounts data download functions, `nc_data_dl()` or `nc_count()`, you have the helpful option of filtering data by region. Let's demonstrate how to use the `region` argument with a few examples.
...
Great, we now know the codes we need and can grab our metadata using `nc_count()`:

```{r}
nc_count(region = list(subnational2 = c("CA-QC-MR", "CA-ON-TO")))
```

Similarly, this function can be used to grab metadata for a bounding box using latitude and longitude coordinates:
...
This function, by default, uses the BSCDATA taxonomic authority and returns all species ids related to the search term including related subspecies. For this reason, it is considered a more robust method for ensuring that you do not miss observations in your search.

This function also gives you the option of returning all species codes (including subspecies) related to a single species, and is considered a more robust method for ensuring that you do not miss observations.
